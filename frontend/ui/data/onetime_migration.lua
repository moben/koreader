--[[
Centralizes any and all one time migration concerns for LuaSettings & DocSettings.
--]]
...
function OnetimeMigration:migrateSettings(config)
    ...
    elseif cfg_mt_idx == LuaSettings then
        ...
        logger.warn("Passed an unsupported object class to OnetimeMigration!")
        return
    end

    -- Retrieve the date of the previous migration, if any
    local last_migration_date = config:readSetting("last_migration_date", 0)
    ...
    if cfg_class == "global" and last_migration_date < 20200421 then
        ...
        if config:hasNot("text_lang_default") and config:hasNot("text_lang_fallback") then
            local g_text_lang_set = false
            local hyph_alg_default = config:readSetting("hyph_alg_default")
            if hyph_alg_default then
                local dict_info = ReaderTypography.HYPH_DICT_NAME_TO_LANG_NAME_TAG[hyph_alg_default]
                if dict_info then
                    config:saveSetting("text_lang_default", dict_info[2])
            ...
            local hyph_alg_fallback = config:readSetting("hyph_alg_fallback")
            if not g_text_lang_set and hyph_alg_fallback then
                ...
                if dict_info then
                    config:saveSetting("text_lang_fallback", dict_info[2])
                    g_text_lang_set = true
            ...
            if not g_text_lang_set then
                ...
                config:saveSetting("text_lang_fallback", ReaderTypography.DEFAULT_LANG_TAG)
            end
        end
    end
    ...
    if cfg_class == "global" and last_migration_date < 20210306 then
        logger.info("Performing one-time migration for 20210306 (1/2)")

        -- Migrate settings from 2021.02 or older.
        if config:readSetting("screensaver_type") == "message" then
            config:saveSetting("screensaver_type", "disable")
            config:makeTrue("screensaver_show_message")
        end
        if config:has("screensaver_no_background") then
            if config:isTrue("screensaver_no_background") then
                config:saveSetting("screensaver_background", "none")
            end
            config:delSetting("screensaver_no_background")
        end
        if config:has("screensaver_white_background") then
            if config:isTrue("screensaver_white_background") then
                config:saveSetting("screensaver_background", "white")
            end
            config:delSetting("screensaver_white_background")
        end
    end
    ...
    if cfg_class == "global" and last_migration_date < 20210306 then
        ...
        local opds_servers = config:readSetting("opds_servers")
        if opds_servers then
            ...
            config:saveSetting("opds_servers", opds_servers)
        end
    end

    -- Statistics, https://github.com/koreader/koreader/pull/7471
    if cfg_class == "global" and last_migration_date < 20210330 then
        ...
        if not ok or not ReaderStatistics then
            ...
            local settings = config:readSetting("statistics")
            if settings then
                ...
                config:saveSetting("statistics", settings)
            end
        end
    end

    -- ScreenSaver, https://github.com/koreader/koreader/pull/7496
    if cfg_class == "global" and last_migration_date < 20210404 then
        logger.info("Performing one-time migration for 20210404")

        -- Migrate settings from 2021.03 or older.
        if config:has("screensaver_background") then
            config:saveSetting("screensaver_img_background", config:readSetting("screensaver_background"))
            config:delSetting("screensaver_background")
        end
    end

    -- Fontlist, cache migration, https://github.com/koreader/koreader/pull/7524
    if cfg_class == "global" and last_migration_date < 20210409 then
    ...
    if cfg_class == "global" and last_migration_date < 20210412 then
    ...
    if cfg_class == "global" and last_migration_date < 20210414 then
    ...
    if cfg_class == "global" and last_migration_date < 20210508 then
    ...
    local function readerfooter_defaults(date)
        ...
        local settings = config:readSetting("footer", ReaderFooter.default_settings)
        ...
        config:saveSetting("footer", settings)
    end

    -- https://github.com/koreader/koreader/pull/7702
    if cfg_class == "global" and last_migration_date < 20210518 then
    ...
    if cfg_class == "global" and last_migration_date < 20210521 then
    ...
    if cfg_class == "global" and last_migration_date < 20210531 then
        logger.info("Performing one-time migration for 20210531")

        if config:has("zoom_mode") then
            -- fontcache may be in an older format, drop it
            drop_fontcache()
            ...
            local zoom_mode_genus, zoom_mode_type = ReaderZooming:mode_to_combo(config:readSetting("zoom_mode"))
            config:saveSetting("kopt_zoom_mode_genus", zoom_mode_genus)
            config:saveSetting("kopt_zoom_mode_type", zoom_mode_type)
            config:delSetting("zoom_mode")
        end
    end

    -- https://github.com/koreader/koreader/pull/7876
    if cfg_class == "global" and last_migration_date < 20210622 then
    ...
    if cfg_class == "global" and last_migration_date < 20210720 then
        ...
        config:saveSetting("duration_format", "classic")
    end

    -- 20210831, Clean VirtualKeyboard settings of disabled layouts, https://github.com/koreader/koreader/pull/8159
    if cfg_class == "global" and last_migration_date < 20210831 then
        logger.info("Performing one-time migration for 20210831")
        local keyboard_layouts = config:readSetting("keyboard_layouts") or {}
        ...
        config:saveSetting("keyboard_layouts", keyboard_layouts_new)
    end

    -- 20210902, Remove unneeded auto_warmth settings after https://github.com/koreader/koreader/pull/8154
    if cfg_class == "global" and last_migration_date < 20210925 then
        logger.info("Performing one-time migration for 20210925")
        config:delSetting("frontlight_auto_warmth")
        config:delSetting("frontlight_max_warmth_hour")
    end

    -- OPDS, add new server, https://github.com/koreader/koreader/pull/8606
    if cfg_class == "global" and last_migration_date < 20220116 then
    ...
    if cfg_class == "global" and last_migration_date < 20220426 then
        local function migrateSettingsName(old, new, factor)
            factor = factor or 1
            if config:readSetting(old) then
                local value = math.floor(config:readSetting(old) * factor)
                config:saveSetting(new, value)
                config:delSetting(old)
            end
        end
    ...
    if cfg_class == "global" and last_migration_date < 20220523 then
        local function migrateSettingsName(old, new, factor)
            factor = factor or 1
            if config:readSetting(old) then
                local value = math.floor(config:readSetting(old) * factor)
    ...
    if cfg_class == "global" and last_migration_date < 20220625 then
    ...
    if cfg_class == "global" and last_migration_date < 20220819 then
    ...
    if cfg_class == "global" and last_migration_date < 20220914 then
    ...
    if cfg_class == "global" and last_migration_date < 20220930 then
    ...
    if cfg_class == "global" and last_migration_date < 20221027 then
    ...
    if cfg_class == "global" and last_migration_date < 20230707 then
        ...
        if calibre_opds and calibre_opds.host and calibre_opds.port then
           ...
           config:saveSetting("opds_servers", opds_servers)
           config:delSetting("calibre_opds")
        end
    end
    ...
    if cfg_class == "global" and last_migration_date < 20230710 then
        ...
        if not ok or not KOSync then
            ...
            local settings = config:readSetting("kosync")
            if settings then
    ...
    if cfg_class == "global" and last_migration_date < 20230901 then
        logger.info("Performing one-time migration for 20230901")

        local contrast = config:readSetting("kopt_contrast")
        if contrast then
            config:saveSetting("kopt_contrast", 1 / contrast)
        end
    end

    -- 20231217, change folder_shortcuts setting from array to hash table, https://github.com/koreader/koreader/pull/11221
    if cfg_class == "global" and last_migration_date < 20231217 then
        logger.info("Performing one-time migration for 20231217")

        local shortcuts = config:readSetting("folder_shortcuts")
        if shortcuts and shortcuts[1] ~= nil then
            ...
            config:saveSetting("folder_shortcuts", new_shortcuts)
        end
    end
    ...
    if cfg_class == "global" and last_migration_date < 20240408 then
        logger.info("Performing one-time migration for 20240408")

        local image_file = config:readSetting("screensaver_type") == "image_file" and config:readSetting("screensaver_image")
        if image_file then
            config:saveSetting("screensaver_type", "document_cover")
            config:saveSetting("screensaver_document_cover", image_file)
        end
    end

    -- https://github.com/koreader/koreader/pull/11999
    if cfg_class == "global" and last_migration_date < 20240616 then
    ...
    if cfg_class == "global" and last_migration_date < 20240731 then
        logger.info("Performing one-time migration for 20240731")

        local settings = config:readSetting("footer")
    ...
    if cfg_class == "global" and last_migration_date < 20240911 then
    ...
    if cfg_class == "global" and last_migration_date < 20240914 then
        logger.info("Performing one-time migration for 20240914")

        local setting = config:readSetting("save_document")
        if setting == "always" then
            config:makeTrue("highlight_write_into_pdf")
        elseif setting == "prompt" then
            config:makeTrue("highlight_write_into_pdf")
            config:makeTrue("highlight_write_into_pdf_notify")
        end
        config:delSetting("save_document")
    end

    -- 20240915, metric_length -> dimension_units, https://github.com/koreader/koreader/pull/12507
    if cfg_class == "global" and last_migration_date < 20240915 then
        logger.info("Performing one-time migration for 20240915")

        if config:has("metric_length") then
            config:saveSetting("dimension_units", config:nilOrTrue("metric_length") and "mm" or "in")
            config:delSetting("metric_length")
        end
    end

    -- 20240928, Profiles auto-execute, https://github.com/koreader/koreader/pull/12564
    if cfg_class == "global" and last_migration_date < 20240928 then
        logger.info("Performing one-time migration for 20240928")

        if config:has("autostart_profiles") then
            local profiles = config:readSetting("autostart_profiles")
            if next(profiles) then
                local autoexec = config:readSetting("profiles_autoexec", {})
                autoexec.Start = autoexec.Start or {}
                for profile in pairs(profiles) do
                    autoexec.Start[profile] = true
                end
            end
            config:delSetting("autostart_profiles")
        end
    end
    ...
    if cfg_class == "global" and last_migration_date < 20241123 then
        logger.info("Performing one-time migration for 20241123")

        local Device = require("device")
        if not Device:isTouchDevice() and config:readSetting("screensaver_delay") == "gesture" then
            config:saveSetting("screensaver_delay", "tap")
        end
    end

    -- 20241207, We moved patch management to core. Remove the original plugin.
    -- https://github.com/koreader/koreader/pull/12862
    if cfg_class == "global" and last_migration_date < 20241207 then
    ...
    if cfg_class == "global" and last_migration_date < 20241208 then
        logger.info("Performing one-time migration for 20241208")

        config:delSetting("kopt_full_screen")
    end

    -- 20241228, Refactor wallabag plugin.
    -- https://github.com/koreader/koreader/pull/12949
    if cfg_class == "global" and last_migration_date < 20241228 then
    ...
    if cfg_class == "global" and last_migration_date < 20250207 then
    ...
    if cfg_class == "global" and last_migration_date < 20250302 then
        logger.info("Performing one-time migration for 20250302")

        local servers = config:readSetting("opds_servers")
        if servers then
            config:delSetting("opds_servers")
    ...
    if cfg_class == "global" and last_migration_date < 20250318 then
    ...
    if cfg_class == "global" and last_migration_date < 20250405 then
        logger.info("Performing one-time migration for 20250405")

        if config:isFalse("show_finished") then
            config:saveSetting("show_filter", {
                status = {
                    new       = true,
                    reading   = true,
                    abandoned = true,
        ...
        config:delSetting("show_finished")
    end
    ...
    config:saveSetting("last_migration_date", CURRENT_MIGRATION_DATE)
end

return OnetimeMigration
